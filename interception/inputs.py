...
def char_shift_key(c):
    if c in UPPER_TO_LOWER:
        return UPPER_TO_LOWER[c]
    return None
...
def listen_to_keyboard() -> int:
    ...
    print("Listening to keyboard, press any keyboard keys, or press ESC if you don't have a keyboard.")
    try:
        while True:
            device = context.wait()
            stroke = context.receive(device)
            ...
            if context.is_keyboard(device) and stroke.code == 0x01:
                print("ESC pressed, exited.")
                return device
            elif context.is_keyboard(device):
                print(f"Received stroke {stroke} on keyboard device {device}")
                return device
    finally:
        context._destroy_context()
...
def listen_to_mouse() -> int:
    ...
    print("Listening to mouse, move or click your mouse, or press ESC if you don't have a mouse")
    try:
        while True:
            device = context.wait()
            stroke = context.receive(device)
